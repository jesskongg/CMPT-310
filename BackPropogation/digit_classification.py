...
# network structure defined by layers parameter
# 784 features (28*28 pixel images), 1 hidden layer with 10 neurons, and 1 output neuron as the output layer
# To obtain simple perceptron network, pass layers = [784,1] to init_network function
# W = weights | W[l][i][j] = weight which connects neuron i in layer l + 1 to neuron j in layer l
# B = biases | B[l][i] = bias for neuron i in layer l + 1
...
def compute_activations(x, W, B):
    ...
    # z = value of each node's input sum
    # a = activation function

        # calculate dot product between the input and their corresponding weights 
        z = np.dot(W[i], a_s[i]) - B[i]
        
        # insert z to existing array of zeros
        z_s[i+1] = z
...
# backpropogation calculates derivative of training error wrt each of network's weights
...
def backpropagation(x, t, W, B):
    ...
    delta = (t - a_s[-1]) * derivative_activation(z_s[-1])
    # print ("delta: ", delta)
    d_b[-1] = -1 * delta
    ...
    for i in range(2, len(W)+1):
        ...
        d_b[-i] = -1 * delta
        d_w[-i] = np.dot(delta,
                                a_s[-i-1].reshape((1,a_s[-i-1].shape[0])))
...
def print_status(print_sample_values, z_s, a_s, d_w, d_b, W, B):
    if print_sample_values and z_s is not None and a_s is not None:
        # print ("z_s[0]: ", z_s[0])
        # print ("z_s[1].shape: ", z_s[1].shape)
        # print ("z_s[2].shape: ", z_s[2].shape)
        # print ("a_s[0].shape: ", a_s[0].shape)
        # print ("a_s[1].shape: ", a_s[1].shape)
        # print ("a_s[2].shape: ", a_s[2].shape)
